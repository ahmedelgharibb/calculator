// Helper: API base URL for local backend
...
function renderStep2() {
      ...
      <form id="fieldsForm" autocomplete="off" class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-xl border border-gray-100">
        ...
        <div id="fieldsList">
          ${calculator.fields.map((f, i) => `
            <div class="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200 relative group">
              <div class="flex items-center gap-2 mb-3">
                <input type="text" value="${f.name}" data-idx="${i}" class="field-name-input font-semibold text-base border border-gray-200 rounded-lg px-3 py-2 flex-1 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition disabled:bg-gray-100 disabled:text-gray-400" maxlength="24" required placeholder="Field label (e.g. Homework, Quiz)" aria-label="Field label (e.g. Homework, Quiz)" ${f.editing ? '' : 'disabled'} />
                <button type="button" class="edit-field-btn text-indigo-500 hover:text-indigo-700 p-2 rounded-full transition ml-1 flex items-center justify-center border border-transparent hover:border-indigo-200 bg-white shadow-sm" data-idx="${i}" aria-label="Edit field">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 10-4-4l-8 8v3zm0 0v3h3" /></svg>
                </button>
  ...
  document.getElementById('addFieldBtn').onclick = () => {
    calculator.fields.push({ name: '', options: [], editing: true });
    renderStep2();
    setTimeout(() => {
  ...
  // Edit field icon logic
  document.querySelectorAll('.edit-field-btn').forEach(btn => {
    btn.onclick = (e) => {
      const idx = btn.getAttribute('data-idx');
      calculator.fields.forEach((f, i) => { f.editing = false; });
      calculator.fields[idx].editing = true;
      renderStep2();
      setTimeout(() => {
        const inp = document.querySelector(`.field-name-input[data-idx='${idx}']`);
        if (inp) inp.focus();
      }, 50);
    };
  });
  ...
  document.querySelectorAll('.field-name-input').forEach(inp => {
    ...
    inp.onblur = (e) => {
      const idx = e.target.getAttribute('data-idx');
      calculator.fields[idx].editing = false;
      renderStep2();
    };
  });
  ...
  document.getElementById('fieldsForm').onsubmit = async (e) => {
    ...
    try {
      // Save the entire calculator at once
      ...
      if (!res.ok) throw new Error(result.error || 'Failed to save calculator');
...
function renderStep3() {
  ...
  document.getElementById('saveCalculatorBtn').onclick = async () => {
    ...
    try {
      ...
      const { data: calcData, error: calcError } = await fetch(`${API_BASE}/calculators`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(calculator)
      });
      ...
      for (let i = 0; i < calculator.fields.length; i++) {
        ...
        const { data: fieldData, error: fieldError } = await fetch(`${API_BASE}/calculators/${calculatorId}/fields`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: field.name, field_order: i })
        });
        ...
        for (let j = 0; j < (field.options || []).length; j++) {
          ...
          const { error: optError } = await fetch(`${API_BASE}/calculators/${calculatorId}/fields/${fieldId}/options`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ label: opt.label, value: opt.value, option_order: j })
          });
...
function renderAbout() {
  appContainer.innerHTML = `
    <section style="max-width:700px;margin:0 auto;padding:48px 0 32px 0;">
      ...
      <p style="font-size:1.15rem;color:#181824;">ScoreCalc lets you build, save, and reuse your own score calculators. It is simple, fast, and works on any device. Great for teachers, students, and anyone who needs to calculate scores easily.</p>
    ...
    document.querySelectorAll('.delete-calc-btn').forEach((btn, idx) => {
      btn.onclick = async (e) => {
        ...
        showDeleteModal(async () => {
          ...
          await fetch(`${API_BASE}/calculators/${calcId}`, { method: 'DELETE' });
          fetchCalculatorsInline();
        });
      };
    });
...
async function showCalculatorInline(id) {
  ...
  try {
    const res = await fetch(`${API_BASE}/calculators/${id}`);
    ...
    function renderTestResult(userName, answers) {
      ...
      attempts.unshift({ user_name: userName, score: total });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  calcForm.onsubmit = async (e) => {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      for (let j = 0; j < (field.options || []).length; j++) {
        const opt = field.options[j];
        await fetch(`${API_BASE}/calculators/${calculatorId}/fields/${fieldId}/options`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ label: opt.label, value: opt.points, option_order: j })
        });
      }
    }
